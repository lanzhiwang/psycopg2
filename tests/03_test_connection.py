...
class ConnectionTests(ConnectingTestCase):
    ...
    def test_close_idempotent(self):  # idempotent 幂等
        conn = self.conn
        conn.close()
        conn.close()
        self.assert_(conn.closed)  # unittest.TestCase.assert_ = unittest.TestCase.assertTrue
    ...
    @skip_if_windows
    def test_cleanup_on_badconn_close(self):  # todo
        # ticket #148
        conn = self.conn
        cur = conn.cursor()
    ...
    def test_reset(self):
        ...
        if self.conn.info.server_version >= 90100:
            conn.deferrable = False  # deferrable 可延期的
